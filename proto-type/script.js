...
$(function() {
	...
	//$('#model').val("spice-and-herbs-dataset");
	//$('#version').val("13");
	//$('#api_key').val("tbGiR3vVrwzLF1k6atxG");

	setupButtonListeners();
});

var infer = function() {
	$('#output').html("Detecting...");
...
var retrieveDefaultValuesFromLocalStorage = function() {
	try {
		var api_key = "tbGiR3vVrwzLF1k6atxG"
		var model = "spice-and-herbs-dataset"
		var format = 13
	...
	$('#format').change(function() {
		localStorage.setItem('rf.format', "image");
	});
};
...
var getSettingsFromForm = function(cb) {
	...
	var parts = [
		"https://detect.roboflow.com/spice-and-herbs-dataset/30?api_key=tbGiR3vVrwzLF1k6atxG"
	];
...
function addItem(json) {
		...
		// Get the first object from the predictions array
		var firstPrediction = data.predictions[0];

		// Get the class of the first object
		var firstClass = firstPrediction.class;

        $('#output2').html("").append([
				firstClass, 
				"is used in the following recepies"
			].join(" "));

		var apiUrl = 'https://api.spoonacular.com/recipes/findByIngredients';

		var apiKey = "d8a86e4173dd40e189746233ef419954";

		var url = `${apiUrl}?ingredients=${encodeURIComponent(firstClass)}&apiKey=${apiKey}`;

		// Making a GET request to the Spoonacular API
		fetch(url)
